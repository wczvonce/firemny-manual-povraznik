#!/usr/bin/env node
/* Over validitu content.json, potom commit+push — BEZ shellu.
 * Použitie:
 *   node scripts/commit-content.js -m "<popis zmeny>"
 * Spúšťať z koreňa repa firemna-prirucka PO úprave content.json.
 * Obsah smie upravovať ktokoľvek s prístupom k zdieľanému účtu (Ivan aj zamestnankyňa) —
 * vedomé rozhodnutie vlastníka, bez kódového slova.
 *
 * Bezpečnosť:
...
// 2) git add: len content.json + presne referencované, overené a existujúce prílohy (M1).
const toAdd = [FILE];
...
// 3) commit (bez shellu; správa ide ako argument, nie do reťazca).
const committed = gitTry(['commit', '-m', msg]);
if (!committed.ok) fail('commit zlyhal (mozno ziadna zmena): ' + committed.err);

// 4) fetch+rebase pred pushom; pri konflikte nič nezverejni (commit zostáva lokálne).
...
// 5) push (ak rebase neskončil konfliktom).
if (pushed) {
